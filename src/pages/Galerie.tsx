...
import { Camera, Video, Play, Cpu, Shield, Hexagon, Sparkles, Image as ImageIcon, X } from 'lucide-react';
          ...
          <div className="flex justify-center mb-8">
            <div className="flex space-x-4 bg-gradient-to-br from-gray-900/80 via-black/90 to-gray-900/80 backdrop-blur-sm p-2 rounded-2xl border border-gray-800">
              {filters.map((filter) => (
                <button
                  key={filter.key}
                  onClick={() => setActiveFilter(filter.key as 'all' | 'images' | 'videos')}
          ...
          {filteredMedia.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredMedia.map((item, index) => (
                <div
                  key={index}
                  className="group relative bg-gradient-to-br from-gray-900/80 via-black/90 to-gray-900/80 backdrop-blur-sm rounded-2xl border border-gray-800 hover:border-red-500/50 transition-all duration-500 transform hover:scale-105 overflow-hidden"
                >
                  {/* Holographic Background */}
                  ...
                  <div className="p-4 relative z-10">
                    <div className="flex items-center justify-between">
                      <h3 className="text-white font-semibold group-hover:text-red-400 transition-colors duration-300 drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">
                        {item.alt}
                      </h3>
